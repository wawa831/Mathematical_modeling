...
#### 1.3.1 运输时间计算
 ...
 - **上/下车时间**：
   ...
   $$T_{on}=n_{on}\times\frac{t_0}{60}$$
   $$T_{off}=n_{off}\times\frac{t_0}{60}$$
 - **每辆车每一段总用时**：
...
#### 8.4.1 数学模型
 ...
 - 约束条件：与问题 3 类似，但候车人数 $D_i^m$ 和运行速率 $V$ 为随机变量，$D_i^m\sim U[0.85D_i^m,1.15D_i^m]$，$V\sim U[0.95V,1.05V]$。
...
## 9.3 问题 3 结果
在可以自由调配车辆路线的情况下，运用遗传算法对早高峰和下课高峰的车辆路线分配进行全局搜索，得到了使两个高峰时段总运输时间最短的最优方案。具体的车辆调度安排以及最短总运输时间将在后续详细给出。通过自由调配路线，能够充分发挥每辆车的运输能力，进一步优化运输效率，减少总运输时间。与前两问固定路线或简单调整路线的情况相比，自由调配路线可以根据不同时段的需求特点，更加灵活地安排车辆，避免了某些路线车辆资源的浪费，提高了整体的运输效益。

## 9.4 问题 4 结果
考虑天气因素对候车人数和运行速率的影响，采用蒙特卡罗模拟方法进行多次仿真计算，得到了在不同天气场景下的最优调配方案。将该方案与问题 3 的结果进行比较分析，以明确候车人数和运行速率变化对决策的影响。

在蒙特卡罗模拟中，随机生成了大量不同的候车人数和运行速率组合，对每种组合进行离散事件仿真，统计运输时间。结果显示，当候车人数增加和运行速率降低时，运输时间明显增加。例如，在某些模拟场景中，候车人数增加 15%且运行速率降低 5%，运输时间比问题 3 的结果增加了约 15% - 20%。这表明天气因素对校车调度方案有显著影响，在实际运营中需要充分考虑这些因素，制定更加灵活的调度策略。

通过比较不同场景下的最优调配方案，可以发现当候车人数增加时，需要增加车辆的投入或者调整车辆的路线，以满足更多学生的需求；当运行速率降低时，需要合理安排车辆的发车间隔，避免车辆在站点过度积压。因此，在实际的校车调度中，应建立实时的天气监测和数据分析系统，根据天气变化及时调整调度方案，以提高校车运营的效率和服务质量。
